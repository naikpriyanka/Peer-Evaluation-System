...
public class Evaluation {
	...
	public void start() {
		...
		/*
		 * The following code makes sure that the user will choose the input
		 * from a combo box instead of entering the digits. This restricts the
		 * user to certain option making it hard for invalid option to enter the
		 * program.
		 */
		...
		submitButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				 ...
				 * Each entry of the map contains the Name as key and a List of
				 * scores as value.
				 */
				Map<String, List<Integer>> rawScoresMap = new LinkedHashMap<String, List<Integer>>();

				for (int i = 0; i < table.getRowCount(); i++) {
					// These two components are extracted from each row of the
					// table.
					List<Integer> rawScores = new LinkedList<Integer>();
					String name = null;

					for (int j = 0; j < table.getColumnCount(); j++) {
						// All values of 0th column are names. Store them as
						// strings.
						if (j == 0) {
							// Get the name
						...
						// Get the scores. Parse them to Integers before storing
						// in map.
